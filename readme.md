...
- All the constraints in the provided SiteUser model must be validated while creating the user in signup page. e.g. `models.CharField(max_length=20)` means a max length of 20 must be checked in the validation layer.
- All attributes must have a None/blank check (after strip) unless mentioned as null=True or blank=True in the model.
- password must be a minimum of 8 characters, max of 20 characters, and cannot be same as username. Password cannot start/end with empty spaces. It'll be stripped before checking for constraints and hashing it. That's the exact requirement, it should be followed strictly.
