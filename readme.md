...
- Username must not contain any uppercase letters, numbers and special characters are allowed. empty spaces are not allowed in between.
- password must be a minimum of 8 characters, max of 20 characters, and cannot be same as username (case sensitive). Password cannot start/end with empty spaces. It cannot contain empty spaces in between as well. That's the exact requirement, it should be followed strictly.
